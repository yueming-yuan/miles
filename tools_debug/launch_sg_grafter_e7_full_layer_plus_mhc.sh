#!/bin/bash
# E7 sg-side. sg's V4 plugin already dumps `layer_input` natively (deepseek_v4.py:1139)
# and `post_norm_hidden` after self.norm (line 1400). No source-patcher needed.
set -ex

OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-e7-full-layer-plus-mhc-sg
mkdir -p "$OUTPUT"
