#!/bin/bash
# A2 — Replace sg.KV-LoRA path only.
#
# **Replaced kernels** (mg uses sg's compute downstream):
#   sg.{wkv, kv_norm, kv_rope}
#
...
#                sg.KV-LoRA path; mg's own attn_q gets overridden)
#
...
# value, but only sg.KV-LoRA's output (attn_q) is sent back to mg. Mg's KV-LoRA / compressor
# etc. remain mg-natural.
set -ex

OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-a2-kv-lora-only-mg
mkdir -p "$OUTPUT"
