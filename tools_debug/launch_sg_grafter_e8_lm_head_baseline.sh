#!/bin/bash
# E8 sg-side. sg's V4 plugin already dumps `layer_input` and `post_norm_hidden`.
# `lm_head_logits` dumped via `LogitsProcessor._get_logits` edit in logits_processor.py.
set -ex

OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-e8-lm-head-baseline-sg
mkdir -p "$OUTPUT"
