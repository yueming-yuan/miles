#!/bin/bash
# E3 (实验 A) — entire attention BLOCK swap (sg side).
# b2t: sg receives mg's attn_q + attn_v, replaces own. sg computes its full attention
# pipeline (indexer + compressor + sparse-MLA / FlashMLA / SWA) on mg's inputs.
# t2b: sg sends sg.attn_output to mg. mg uses sg's value downstream.
set -ex

OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-e3-attn-block-sg
mkdir -p "$OUTPUT"
