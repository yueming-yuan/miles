#!/bin/bash
# E6 sg-side. sg's V4 plugin already dumps `layer_input` natively
# (deepseek_v4.py:1139), no source-patcher needed.
set -ex

OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-e6-full-layer-baseline-sg
mkdir -p "$OUTPUT"
