#!/bin/bash
# A5 — Replace ENTIRE attention block including output projection (input_layernorm → mqa_wo_b_out).
#
# **Replaced kernels** (mg uses sg's compute downstream):
#   sg.{Q-LoRA + KV-LoRA + compressor + indexer + sparse-MLA + wo_a + wo_b LoRA output projection}
#
...
OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-a5-attn-block-with-output-proj-mg
mkdir -p "$OUTPUT"
