#!/bin/bash
# A3 — Replace sg.Q-LoRA + KV-LoRA paths combined.
#
# **Replaced kernels** (mg uses sg's compute downstream):
#   sg.{wq_a, q_lora_norm, wq_b, q_heads_norm, q_rope, wkv, kv_norm, kv_rope}
#
...
#   t2b (sg→mg): `attn_q + attn_v` (sg's post-RoPE Q, computed on mg's input_layernorm output via
...
OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-a3-q-kv-lora-mg
mkdir -p "$OUTPUT"
