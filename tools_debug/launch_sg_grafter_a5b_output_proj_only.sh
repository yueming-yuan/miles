#!/bin/bash
# A5' sg-side. Both attn_output (line 907) and mqa_wo_b_out (line 943) natively dumped
# in sg's V4 plugin. Idle DP path now also fires empty mqa_wo_a_out + mqa_wo_b_out
# (deepseek_v4.py:864 area) so collective syncs across all 8 sg ranks.
set -ex

OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-a5b-output-proj-only-sg
mkdir -p "$OUTPUT"
