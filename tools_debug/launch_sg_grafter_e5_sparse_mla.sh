#!/bin/bash
# E5 — sparse-MLA-kernel approximation (sg side).
# Same filter as mg side; sg sends/receives the same names with role=target.
set -ex

OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-e5-sparse-mla-sg
mkdir -p "$OUTPUT"
