#!/bin/bash
# A1 sg side. sg's V4 plugin already dumps `input_layernorm` (deepseek_v4.py:1146) and
# `attn_q` (deepseek_v4.py:800), no source-patcher needed. Lm_head_logits dump (added
# in commit X for E8) is orthogonal to A1's filter.
set -ex

OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-a3-q-kv-lora-sg
mkdir -p "$OUTPUT"
...
export DUMPER_GRAFTER_T2B_FILTER="${DUMPER_GRAFTER_T2B_FILTER:-name in (\"attn_q\", \"attn_v\") and graft_phase == \"prefill\"}"
