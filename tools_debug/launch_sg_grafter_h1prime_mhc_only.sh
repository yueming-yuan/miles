#!/bin/bash
# H1 sg-side. sg's V4 plugin natively dumps:
# - layer_input (deepseek_v4.py:1139, every layer)
# - hc_attn_pre, hc_attn_post (lines 1144, 1156)
# - hc_ffn_pre, hc_ffn_post (lines 1161, 1213)
# - attn_output (line 907)
# - mlp_output (line 1202)
# All needed graft tensors already exist on sg side. No source-patcher needed.
set -ex

OUTPUT=/storage/yueming/dumper-out/v4-iter59-grafter-h1prime-mhc-only-sg
mkdir -p "$OUTPUT"
...
export DUMPER_GRAFTER_B2T_FILTER="${DUMPER_GRAFTER_B2T_FILTER:-name in (\"layer_input\", \"mqa_wo_b_out\", \"mlp_output\") and graft_phase == \"prefill\"}"
